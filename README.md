...
Ledgerline is a payment operations SPA for monitoring and managing payment transactions through the companion .NET Clean Architecture Web API in `C:\source\payments`. It provides a secure login flow, a responsive transaction ledger, payment creation with client-side idempotency, pagination, and actions for completing or cancelling pending payments.
